...
{
    ESP_LOGI(TAG, "Audio encoder task started on core %d", xPortGetCoreID());
    ...
    while (1) {
        ...
        if (pcm_frame) {
            // Encode the 20ms frame
            int enc_len = opus_encode(encoder, pcm_frame, FRAME_SAMPLES, enc_buffer + 12, sizeof(enc_buffer) - 12);
            vRingbufferReturnItem(pcm_in, pcm_frame);

            if (enc_len > 0) {
                // Construct RTP Header (12 bytes)
                enc_buffer[0] = 0x80; // Version 2
                enc_buffer[1] = 96;   // Payload Type (Dynamic, 96 for Opus)
                ...
                xRingbufferSend(rtp_out, enc_buffer, enc_len + 12, portMAX_DELAY);
            }
        }
    }
}
...
esp_err_t audio_encoder_init(RingbufHandle_t pcm_in_buf, RingbufHandle_t rtp_out_buf)
{
    ...
    // Pinned to Core 1
    xTaskCreatePinnedToCore(audio_encoder_task, "audio_enc", 8192, NULL, configMAX_PRIORITIES - 2, NULL, 1);
    return ESP_OK;
}
